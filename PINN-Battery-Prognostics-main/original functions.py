...
class SeversonBattery:
    def __init__(self, data_addr, seq_len):  ##初始化，addr说明数据是mat格式
        self.data = scipy.io.loadmat(data_addr)
        self.seq_len = seq_len
        self.steps_slices = 1

        self.features = self.data['Features_mov_Flt']  # 从加载的数据中提取特征 features、剩余使用寿命 RUL、容量损失百分比 PCL 和循环次数 cycles
        self.RUL = self.data['RUL_Flt']
        self.PCL = self.data['PCL_Flt']
        self.cycles = self.data['Cycles_Flt']

        self.idx_train_units = self.data['train_ind'].flatten() - 1  # 从数据集中获取训练、验证和测试单元的索引，并将其转换为一维数组，索引减 1 是为了将索引从 1 开始转换为从0开始
        self.idx_val_units = self.data['test_ind'].flatten() - 1
        self.idx_test_units = self.data['secondary_test_ind'].flatten() - 1

        self.inputs = np.hstack((self.features, self.cycles.flatten()[:, None]))  # 将特征和循环次数组合成输入数据 inputs
        self.targets = np.hstack((self.PCL, self.RUL))  # 将容量损失百分比和剩余使用寿命组合成目标数据 targets
        self.inputs_dim = self.inputs.shape[1]  # 记录输入和目标数据的维度
        ...
        self.inputs_units, self.targets_units = create_units(
            ...
            len_units=np.squeeze(self.num_cycles_all[:, None])  # 将输入和目标数据按电池单元进行划分
        )
        ...
        # 分别从划分好的单元数据中提取训练、验证和测试单元的输入和目标数据。
        ...
        self.targets_train_ndarray = np.zeros((self.num_slices_train, self.seq_len, self.targets_dim))  # 分别计算训练、验证和测试数据的切片总数
        idx_start = 0
        for i in range(len(self.inputs_train_slices)):
            ...
            idx_start += self.num_slices_lib_train[i]  # 创建零数组用于存储切片数据，并将切片数据填充到数组中
        ...
        # 将训练、验证和测试的输入和目标数组转换为 PyTorch 张量，数据类型为 torch.float32
...
def create_units(data, t, RUL, num_units, len_units):  # 将输入的数据按照不同的电池单元进行划分，最终返回划分好的数据列表和对应的RUL列表
...
def create_slices(data_units, RUL_units, seq_len_slices, steps_slices):
    data_slices = []  # 用于存储每个电池单元划分后的特征数据切片
    RUL_slices = []  # 用于存储每个电池单元划分后的剩余使用寿命数据切片
    num_slices = np.zeros(len(data_units), dtype=np.int)  # 一个一维数组，用于记录每个电池单元划分出的切片数量，初始值都设为0
    for i in range(len(data_units)):
        num_slices_tmp = int((data_units[i].shape[0] - max(seq_len_slices, steps_slices))
                             / steps_slices) + 1  # 计算每个unit的slice数量
    ...
    ##将电池单元的数据划分为固定长度的序列切片，方便后续用于机器学习模型的训练和评估

# 根据给定的电池单元索引，从已有的数据集中挑选出对应的训练、验证和测试数据，并且将这些数据进行划分和合并，最终以字典形式返回处理好的输入和目标数据的张量
...
def standardize_tensor(data, mode, mean=0, std=1):
    ...
    ##标准化后的张量。通常会先使用 fit 模式计算训练数据的均值和标准差，然后使用这些统计量对训练数据和测试数据进行 transform 模式的标准化处理
...
class Neural_Net(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, layers, activation='Tanh'):  ##初始化
        super(Neural_Net, self).__init__()  # 调用父类nn.Module的初始化方法，这是在自定义神经网络模块时必须执行的步骤，用于初始化模块的基本属性。
        # 定义网络层
        ...
        nn.init.xavier_normal_(self.layers[-1].weight)  # 初始化线性层的权重

        if activation == 'Tanh':
            self.layers.append(nn.Tanh())
        elif activation == 'Sin':
            self.layers.append(Sin())
        self.layers.append(nn.Dropout(p=0.2))  # 添加一个Dropout层，随机丢弃 20% 的神经元连接

        # 构建中间隐藏层
        for l in range(len(layers) - 1):
            self.layers.append(nn.Linear(in_features=layers[l], out_features=layers[l + 1]))  # 每个隐藏层包含一个线性层，其输入维度为上一层的神经元数量layers[l]，输出维度为当前层的神经元数量layers[l + 1]。
            nn.init.xavier_normal_(self.layers[-1].weight)  # 对每个线性层的权重进行xavier_normal_初始化
            # 根据激活函数类型添加相应的激活函数和Dropout层
            if activation == 'Tanh':
                self.layers.append(nn.Tanh())
            elif activation == 'Sin':
                self.layers.append(Sin())
        ...
        # 构建输出层
        self.layers.append(nn.Linear(in_features=layers[l + 1], out_features=outputs_dim))  # 添加输出层，将最后一个隐藏层的输出映射到输出维度outputs_dim
        nn.init.xavier_normal_(self.layers[-1].weight)  # 对输出层的权重进行初始化
        # 使用nn.Sequential将所有层组合成一个有序的神经网络模型self.NN
        self.NN = nn.Sequential(*self.layers)
    # 前向传播方法
    def forward(self, x):
        self.x = x
        self.x.requires_grad_(True)  ##启用对输入数据x的梯度计算，这对于反向传播更新模型参数是必要的
        self.x_2D = self.x.contiguous().view((-1, self.inputs_dim))  # 将输入数据x转换为二维张量，其中第一维的大小根据数据总量自动推断，第二维为输入特征维度inputs_dim
        NN_out_2D = self.NN(self.x_2D)  ##将二维输入数据x_2D输入到神经网络模型self.NN中进行前向传播计算
        self.u_pred = NN_out_2D.contiguous().view((-1, self.seq_len, self.outputs_dim))  # 将神经网络的输出NN_out_2D转换回与输入数据具有相同序列长度seq_len的三维张量，作为模型的预测输出self.u_pred

        return self.u_pred

##继承自nn.Module，用于实现数据驱动的神经网络模型
class DataDrivenNN(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, layers, scaler_inputs, scaler_targets):
        ...
        self.surrogateNN = Neural_Net(
            ...
            layers=layers
        )  ##初始化一个代理surrogateNN，作为数据驱动神经网络的主体部分，用于学习输入数据与输出之间的映射关系

    def forward(self, inputs):
        s = inputs[:, :, 0: self.inputs_dim - 1]
        t = inputs[:, :, self.inputs_dim - 1:]  ##将输入数据inputs拆分为两部分：s包含除最后一维之外的所有特征，t包含最后一维特征
        ...
        ##将标准化后的s_norm和t_norm在维度 2 上拼接，然后输入到self.surrogateNN中进行前向传播，得到标准化的输出U_norm
        U_norm = self.surrogateNN(x=torch.cat((s_norm, t_norm), dim=2))
        # 对U_norm进行反标准化处理，得到实际的预测结果U，使其恢复到原始数据的尺度
