cap_nom = 1.1;%%设置电池的容量为1.1
unit_num = length(batch_combined);%unit_num表示电池单元的数量，batch_combined是包含多组电池数据的数组
Cycles = cell(unit_num, 1);%初始化为空单元格cell
Tavg = cell(unit_num, 1);
Qd = cell(unit_num, 1);
Tavg_all = zeros(unit_num, 1);%初始化为一个数值数组，用来存储每个电池单元的平均温度
SoH = cell(unit_num, 1);
PCL = cell(unit_num, 1);
Features = cell(unit_num, 1);
RUL = cell(unit_num, 1);

Features_Flt = [];%初始化为空数组
SoH_Flt = [];
PCL_Flt = [];
Cycles_Flt = [];
RUL_Flt = [];
Num_Cycles_Flt = zeros(unit_num, 1);%初始化一个数组，用于存储每个电池单元的循环次数
...
for i = 1:unit_num,
    ...
    end%数据清洗，如果某个放电量超出均值±3倍标准差的范围，将其替换为相邻值的平均值
