...
for round in range(num_rounds):
    ...
    log_sigma_u = torch.zeros((), device=device, requires_grad=True)
    log_sigma_f = torch.zeros((), device=device, requires_grad=True)
    log_sigma_f_t = torch.zeros((), device=device, requires_grad=True)

    criterion = func.My_loss(mode='AdpBal')

    params = list(model.parameters()) + [log_sigma_u, log_sigma_f, log_sigma_f_t]
...
results['lambda_F_t'] = results_epoch['var_F_t']
