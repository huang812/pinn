...
gru_num_layers_list = settings['gru_num_layers']  # GRU层数
gru_hidden_size_list = settings['gru_hidden_size']  # GRU隐藏层维度
...
metric_mean = dict()
metric_std = dict()
metric_mean['train'] = np.zeros((len(gru_num_layers_list), len(gru_hidden_size_list)))
metric_mean['val'] = np.zeros((len(gru_num_layers_list), len(gru_hidden_size_list)))
metric_mean['test'] = np.zeros((len(gru_num_layers_list), len(gru_hidden_size_list)))
metric_std['train'] = np.zeros((len(gru_num_layers_list), len(gru_hidden_size_list)))
metric_std['val'] = np.zeros((len(gru_num_layers_list), len(gru_hidden_size_list)))
metric_std['test'] = np.zeros((len(gru_num_layers_list), len(gru_hidden_size_list)))

for l, gru_num_layers in enumerate(gru_num_layers_list):
    for n, gru_hidden_size in enumerate(gru_hidden_size_list):
        np.random.seed(1234)
