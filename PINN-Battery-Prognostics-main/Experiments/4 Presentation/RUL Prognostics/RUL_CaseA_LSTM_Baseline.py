...
# 沿用 settings 中的参数列表，只是在这里作为 LSTM 参数使用
...
for l, lstm_num_layers in enumerate(lstm_num_layers_list):
    for n, lstm_hidden_size in enumerate(lstm_hidden_size_list):
        ...
        print(f"Train RMSE mean: {metric_mean['train'][l, n]:.6f}, std: {metric_std['train'][l, n]:.6f}")
        print(f"Val   RMSE mean: {metric_mean['val'][l, n]:.6f}, std: {metric_std['val'][l, n]:.6f}")
        print(f"Test  RMSE mean: {metric_mean['test'][l, n]:.6f}, std: {metric_std['test'][l, n]:.6f}")
        print('=' * 80)

# 用最优模型在测试集上输出结果
best_model.eval()
inputs_test = best_inputs_dict['test'].to(device)
targets_test = best_targets_dict['test'][:, :, 1:].to(device)
...
torch.save(
    results,
    '..\\..\\..\\Results\\4 Presentation\\RUL Prognostics\\RUL_CaseA_LSTM_Baseline.pth'
)

print('\nBest model summary:')
print(f'Best validation RMSE: {best_val_metric:.6f}')
print(f'Best LSTM layers: {best_lstm_num_layers}')
print(f'Best LSTM hidden size: {best_lstm_hidden_size}')
print('Results saved to: ..\\..\\..\\Results\\4 Presentation\\RUL Prognostics\\RUL_CaseA_LSTM_Baseline.pth')