...
class SeversonBattery:
    def __init__(self, data_addr, seq_len):  ##初始化，addr说明数据是mat格式
        ...
        self.idx_train_units = self.data[
                                   'train_ind'].flatten() - 1  # 从数据集中获取训练、验证和测试单元的索引，并将其转换为一维数组，索引减 1 是为了将索引从 1 开始转换为从0开始
        ...
        self.targets_train_ndarray = np.zeros(
            (self.num_slices_train, self.seq_len, self.targets_dim))  # 分别计算训练、验证和测试数据的切片总数
        idx_start = 0
...
class Neural_Net(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, gru_num_layers, gru_hidden_size, activation='Tanh'):
        ...
        # 添加第一个GRU层
        self.gru_layers.append(nn.GRU(input_size=inputs_dim, hidden_size=gru_hidden_size, batch_first=True))
        # 激活函数层
        if activation == 'Tanh':
        ...
        # 构建中间GRU层
        for _ in range(1, gru_num_layers):
            self.gru_layers.append(nn.GRU(input_size=gru_hidden_size, hidden_size=gru_hidden_size, batch_first=True))
            # 激活函数层
            if activation == 'Tanh':
                self.layers.append(nn.Tanh())
            elif activation == 'Sin':
        ...
        # 构建输出层
        self.layers.append(
            nn.Linear(in_features=gru_hidden_size, out_features=outputs_dim))
        nn.init.xavier_normal_(self.layers[-1].weight)

        # 使用nn.Sequential将所有层组合成一个有序的神经网络模型self.NN
        self.activation_layers = nn.Sequential(*self.layers)
    ...
    def forward(self, x):
        ...
        for gru_layer in self.gru_layers:
            x, _ = gru_layer(x)
        # 通过激活函数和Dropout层
        NN_out = self.activation_layers(x)
        self.u_pred = NN_out

        return self.u_pred

class Neural_Net_LSTM(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, lstm_num_layers, lstm_hidden_size, activation='Tanh'):
        ...
        # 其余 LSTM 层
        for _ in range(1, lstm_num_layers):
            self.lstm_layers.append(
                nn.LSTM(
...
class DataDrivenLSTM(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, lstm_num_layers, lstm_hidden_size, scaler_inputs, scaler_targets):
        super(DataDrivenLSTM, self).__init__()
        self.seq_len = seq_len
        self.inputs_dim = inputs_dim
        self.outputs_dim = outputs_dim
    ...
    def forward(self, inputs):
        ...
        # 计算 U 对 t 的导数，保持与 DataDrivenNN 一致接口
...
class DataDrivenNN(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, gru_num_layers, gru_hidden_size, scaler_inputs, scaler_targets):
...
class VerhulstPINN(nn.Module):
    def __init__(self, seq_len, inputs_dim, outputs_dim, gru_num_layers, gru_hidden_size, scaler_inputs,
                 scaler_targets):
...
class My_loss(nn.Module):
    ...
    def forward(self, outputs_U, targets_U, outputs_F, outputs_F_t, log_sigma_u, log_sigma_f, log_sigma_f_t):
        ...
        if self.mode == 'AdpBal':
            loss = torch.exp(-log_sigma_u) * loss_U + torch.exp(-log_sigma_f) * loss_F + torch.exp(
                -log_sigma_f_t) * loss_F_t + \
