...
baseline_results = torch.load(
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Results', '4 Presentation', 'RUL Prognostics', 'RUL_CaseB_Baseline.pth'),
    map_location='cpu',
    weights_only=False
)

lstm_results = torch.load(
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Results', '4 Presentation', 'RUL Prognostics', 'RUL_CaseB_LSTM_Baseline.pth'),
    map_location='cpu',
    weights_only=False
)

adpbal_results = torch.load(
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Results', '4 Presentation', 'RUL Prognostics', 'RUL_CaseB_DeepHPM_AdpBal.pth'),
    map_location='cpu',
    weights_only=False
)

sum_results = torch.load(
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Results', '4 Presentation', 'RUL Prognostics', 'RUL_CaseB_DeepHPM_Sum.pth'),
    map_location='cpu',
    weights_only=False
)
...
plt.title('Remaining Useful Life Prediction Comparison (Case B)')
...
save_path = os.path.join(save_dir, "RUL_CaseB_Prediction_Comparison.png")
