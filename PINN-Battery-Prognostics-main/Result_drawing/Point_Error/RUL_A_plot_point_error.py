...
# 结果文件目录
# =========================
BASE_DIR = r'D:\PycharmProjects\PINN-Battery-Prognostics-main\Results\4 Presentation\RUL Prognostics'

results_paths = {
    "GRU": os.path.join(BASE_DIR, "RUL_CaseA_DeepHPM_AdpBal.pth"),
    "LSTM": os.path.join(BASE_DIR, "RUL_CaseA_LSTM_Baseline.pth"),
    "DeepHPM-Sum": os.path.join(BASE_DIR, "RUL_CaseA_DeepHPM_Sum.pth"),
    "DeepHPM-AdpBal": os.path.join(BASE_DIR, "RUL_CaseA_Baseline.pth")
}
...
sum_results = torch.load(results_paths["DeepHPM-Sum"], map_location='cpu', weights_only=False)
adpbal_results = torch.load(results_paths["DeepHPM-AdpBal"], map_location='cpu', weights_only=False)
...
elif 'cycles' in gru_results:
    x = np.array(gru_results['cycles']).squeeze()
elif 'cycle' in gru_results:
    x = np.array(gru_results['cycle']).squeeze()
else:
...
plt.plot(x, err_sum, label='GRU-DeepHPM (Sum)', linestyle='--', color='#ff7f0e', linewidth=1.8)
plt.plot(x, err_adpbal, label='GRU-DeepHPM (AdpBal)', linestyle='-', color='#d62728', linewidth=2.0)
...
plt.title('Pointwise Absolute Error Comparison of RUL Prediction')
...
# =========================
plt.savefig(
    os.path.join(BASE_DIR, 'RUL_CaseA_Pointwise_Absolute_Error_Comparison.png'),
    dpi=600,
    bbox_inches='tight'
)
